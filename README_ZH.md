...
- [x] **列表视图**：文件以表格形式排列，展示文件名、最后修改时间和大小
- [x] **身份认证**：可选的 HTTP Basic 认证
- [x] **凭证持久化**：账号和密码可以保存到 `localStorage`
...
- [webdav-client](https://github.com/perry-mitchell/webdav-client)：直接从浏览器发送 WebDAV 请求，包括列目录和获取元数据。
- [localStorage](https://developer.mozilla.org/zh-CN/docs/Web/API/Window/localStorage)：将连接信息和凭证保存在本机，便于快速访问和切换主机。
- [Bootstrap](https://getbootstrap.com/)：提供 UI 组件、栅格和响应式样式。
- [Alpine.js](https://alpinejs.dev/)：以声明式的方式管理界面状态和用户交互。
